...
const mnuusers=document.getElementById("mnuusers");
 ...
 btnpost.addEventListener("click", ()=>{
    ...
    .then (json=>{ 
        ...
        json.forEach(pub=>{
            res+=`<h3>${pub.title}</h3> 
                  <p>${pub.body}</p>
                  <button type="button" onClick="com(${pub.id})"> Cargar comentarios </button>
                  <div id="comen${pub.id}"></div>`
        })
 ...
 function com (comen){
      ...
      document.getElementById("comen"+ comen).innerHTML="<h4>Comentarios</h4>"+fre;
    })
}
